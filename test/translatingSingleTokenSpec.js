describe( "adding tokens", function() {
	...
	it( "can get a top level translation", function() {
		expect( i18n.translate( 'confirmed' ) ).toEqual( 'It\'s confirmed!' );
	} );

	it( "can get a translation one level deep", function() {
		expect( i18n.translate( 'booking.to_confirm' ) ).toEqual( 'Your booking will be confirmed shortly!' );
	} );

	it( "can get a translation two levels deep", function() {
		expect( i18n.translate( 'booking.hotel.to_confirm' ) ).toEqual( 'Your hotel booking will be confirmed shortly!' );
	} );

	it( "can gets a booking one level higher when not available", function() {
		expect( i18n.translate( 'booking.please_confirm' ) ).toEqual( 'Please confirm your booking' );
	} );

	it( "can gets a booking two levels higher when not available", function() {
		expect( i18n.translate( 'booking.hotel.please_confirm' ) ).toEqual( 'Please confirm your booking' );
	} );

	it( "can replace a token correctly in translation", function() {
		expect( i18n.translate( 'booking.hotel.confirmed', {
			hotel: 'TheHotel'
		} ) ).toEqual( 'Your hotel booking at TheHotel is confirmed!' );
	} );

	it( "can replace multiple tokens correctly in translation", function() {
		expect( i18n.translate( 'booking.confirmed', {
			booking_no: '12432',
			day: 'Sunday'
